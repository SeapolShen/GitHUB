...
using System.Data.SqlClient; // BY PZHO
...
namespace CyBLE_MTK_Application
{
    public partial class CyBLE_MTK : Form
    {
        ...
        //函数定义 int TestResult(string barCode);
        //输入：string 条码号
        /*输出：整数 -1 -- 该条码未测试
                 0 --测试通过
                 1...n:错误码(测试未通过)
       */
        public void WriteTestResult(string barCode, int result)
        {
            SqlConnection Con;
            ...
            {
                MessageBox.Show("connect log server failed" + ex.Message);
            }

            Logger.PrintLog(this, "Test result uploaded.", LogDetailLevel.LogEverything);
            return;
        }
        ...
        public void UploadResult()
        {
            ...
            try
            {
                for (int i = 0; i < DUTInfoDataGridView.Rows.Count; i++)
                {
                    ...
                    WriteTestResult(SerailNumber, result);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("update test log error" + ex.Message);
            }
